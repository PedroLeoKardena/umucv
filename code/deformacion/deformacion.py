...
def my_arguments(parser):
    ...
    parser.add_argument('--imagen', help='Imagen de la que tomamos las referencias y a la que le aplicaremos la deformación.', type=str, required=True)
...
puntos_pixeles_orig = refs[:, :2].astype(np.float32)
...
#Normalizado de imagen para imagenes con resoluciones elevadas.
#Evitamos problemas con el mostrado de imagenes.
ancho_deseado = 640
...
if ancho > ancho_deseado:
    ...
    frame = cv.resize(frame, (ancho_deseado, nuevo_alto))
else:
    escala = 1.0

puntos_pixeles = puntos_pixeles_orig * escala
...
# Pre-calcular imagen rectificada estática
